...
def decision_fun(gp_mean, gp_std):
    shape = gp_mean.shape
    res = np.empty(shape)
    p = np.ones(shape) - stats.norm.cdf(x=np.full(shape,
                                                  THRESHOLD), loc=gp_mean, scale=gp_std)
    curr = gp_mean - 0.2 * gp_std
    res = np.where(np.logical_and(np.greater(THRESHOLD, curr),
                   np.greater(p, 0.3)), THRESHOLD, curr)
    return res
...
class Model(object):
    ...
    def __init__(self, c, h, nu):
        ...
        self.kernel = ConstantKernel(
            c, constant_value_bounds='fixed')*Matern(h, length_scale_bounds='fixed')
        self.gp = GaussianProcessRegressor(
            kernel=self.kernel, random_state=0, optimizer='fmin_l_bfgs_b')  # use default kernel
...
def cv(parameters, x, y):
    k = 5
    kf = KFold(n_splits=k)
    errors = np.empty(shape=len(parameters))
    i = 0
    for (c, h, nu) in tqdm(parameters):
        error = 0
        for train_index, test_index in kf.split(x):
            x_train, x_test = x[train_index], x[test_index]
            y_train, y_test = y[train_index], y[test_index]
            model = Model(c=c, h=h, nu=nu)
            model.fit_model(x_train, y_train)
            y_predicted, y_mean, y_std = model.predict(x_test)
            error += cost_function(y_test, y_predicted)
        errors[i] = error/k
        print(f'\nc, h, nu: {c}, {h}, {nu}, error: {errors[i]}\n')
        i += 1
    index = np.argmin(errors)
    print(errors[index])
    print(parameters[index])
    return parameters[index], index, parameters, errors
...
def main():
    ...
    c = [2**x for x in range(3, 7)]  # first round in [2^-1, 2^4]
    h = [2**x for x in range(-6, -3)]  # first round in [2^-5, 2^1]
    nu = [0.5, 1.5, 2.5, float('inf')]
    parameters = list(product(c, h, nu))
    param = cv(parameters, train_x, train_y)
    print(param)
