...
class Model(object):
    ...
    def __init__(self):
        ...
        self.kernels = {'comp': CompoundKernel, 'Mat15': Matern(nu=1.5)}
        self.model = GaussianProcessRegressor(self.kernels[kernel])
    ...
    def fit_model(self, train_x: np.ndarray, train_y: np.ndarray):
        ...
        #     self.model.fit(train_x, train_y)
        #     mll = self.model.log_marginal_likelihood()
        #     score = self.model.score(train_x, train_y)
        #     print('Kernel %-10s\t\tIter %d/%d - Loss: %.3f    noise: %.3f' % (
        #         kernel, iteration, MAX_ITER, mll, score))
